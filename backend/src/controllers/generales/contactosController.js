...
exports.crear = async (req, res) => {
  ...
  try {
    const result = await db(req).query(
      `INSERT INTO generales.contactos (cliente_id, nombre, telefono, email, whatsapp, cargo)
...
exports.actualizar = async (req, res) => {
  ...
  try {
    const result = await db(req).query(
      `UPDATE generales.contactos SET nombre=$1, telefono=$2, email=$3, whatsapp=$4, cargo=$5, activo=$6
...
exports.eliminar = async (req, res) => {
  try {
    const result = await db(req).query("DELETE FROM generales.contactos WHERE id=$1 RETURNING id", [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: "Contacto no encontrado" });
