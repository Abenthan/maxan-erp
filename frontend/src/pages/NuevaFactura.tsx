...
export default function NuevaFactura() {
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setLoading(true);
    try {
      const json = await api.postXml<FacturaData & { ya_existe?: boolean; factura_existente_id?: number }>(
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (msg.includes("duplicado") || msg.includes("ya está registrada")) {
        setDuplicadoId(factura.factura_existente_id || null);
        return;
      }
      setError(msg);
    } finally {
  ...
  return (
    <div className="max-w-4xl">
      {puedeCrear ? (
        ...
        <div className="p-8 text-center text-red-600">No tienes permiso para crear facturas</div>
      )}
    </div>
  );
}
